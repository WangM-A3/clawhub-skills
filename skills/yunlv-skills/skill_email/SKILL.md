...
metadata:
  yunlv:
    ...
    tags: ["外贸邮件", "商务邮件", "多语言", "开发信", "询盘回复", "B2B邮件", "邮件营销", "邮件撰写"]
    requires:
      env:
        - TRADEGPT_API_KEY
        - EMAIL_API_KEY  # 可选，用于直接发送
      bins:
        - python3
    apis:
    ...
    pricing:
      free:
        features: ["每天20封邮件生成", "5种语言", "基础模板"]
      basic:
        price: 149
        currency: CNY
        period: month
        features: ["无限邮件生成", "15种语言", "邮件序列", "个性化定制", "语气调整"]
      pro:
        price: 399
        currency: CNY
        period: month
        features: ["自动发送", "A/B测试", "回复分析", "智能语气适应", "优先处理队列"]
triggers:
  - "邮件撰写"
  - "外贸邮件"
  - "开发信"
...
3. **内容生成**：TradeGPT生成完整邮件（Subject + Body + Signature）
...
### 数据处理原则
- **邮件内容保护**：生成和发送的邮件内容不存储在第三方服务器
- **SMTP凭证保护**：邮件发送凭证不写入日志或输出文件
- **最小化留存**：发送记录保留30天，之后自动归档

### 权限边界声明
- ✅ **允许**：读取 `./skills/yunlv-skills/references/` 下的参考文件
- ✅ **允许**：调用云旅AI TradeGPT API生成邮件内容
- ✅ **允许**：写入 `./data/yunlv-skills/emailWriter/` 邮件记录
- ✅ **允许**：通过用户配置的SMTP/Email API发送邮件
- ❌ **禁止**：存储或使用客户的邮件地址进行非授权营销
- ❌ **禁止**：自动爬取互联网邮箱地址进行批量发送
