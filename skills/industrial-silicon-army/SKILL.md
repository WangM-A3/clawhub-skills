...
description: >-
  ...
  Use when generating B2B quotation proposals, inventory planning suggestions, or supplier recommendations.
  Use when optimizing procurement, production scheduling, or sales workflows.
  Use when analyzing manufacturing KPIs or cost accounting.
  Use when user mentions "塑化", "塑料原料", "PP粒子", "改性料", "报价方案", "库存规划".
...
- [ ] 报价单包含最新原料价格（参考1688/LookingPlas公开信息校验）
- [ ] 库存规划阈值设置符合企业实际安全库存标准
- [ ] 生产排产结果经过交期可行性验证
- [ ] 幕僚长汇总结果逻辑正确，无信息丢失
...
## 七、Security & Privacy（安全与隐私）

### 存储根路径
- 数据存储根目录：`./data/industrial-silicon-army/`
- 输出目录：`./data/industrial-silicon-army/output/`
- 配置目录：`./config/industrial-silicon-army/`

### 数据处理原则
1. **敏感数据脱敏**
   - 供应商财务信息（账号、税号、银行账户）：仅显示脱敏后四位
   - 客户信用数据：仅返回风险等级，不存储原始数据
   - 采购价格：仅用于计算分析，不明文存储
...
| 能力 | 权限说明 |
|------|----------|
| 1688平台数据 | 参考公开的供应商、商品、价格信息 |
| 采购建议 | **仅供人工决策参考** |
| 财务付款 | **禁止** - 不支持任何自动扣款、转账操作 |
| 数据导出 | 仅允许导出脱敏后的业务报表 |

### 采购建议确认机制
- 所有采购建议均标记为「待确认」状态
- 建议内容包括：供应商名称、价格区间、数量范围
- 最终下单决策由人工确认并执行
- 系统不存储任何采购凭证或支付信息

### 敏感字段处理示例
```
原始数据: {"supplier": "华锦化工", "account": "6217002345678901234", "tax": "91330000123456789X"}
处理后:   {"supplier": "华锦化工", "account": "****1234", "tax": "****6789X"}
```
